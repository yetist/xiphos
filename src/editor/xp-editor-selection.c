...
static void webview_typing_attributes_changed (WebKitEditorState *editor, GParamSpec *spec, XpEditorSelection *selection)
{
	...
	check_and_update_typing_attr (selection, attrs, WEBKIT_EDITOR_TYPING_ATTRIBUTE_UNDERLINE, "underline");
...
static void editor_selection_set_webview (XpEditorSelection *selection, WebKitWebView *webview)
{
	...
	if (selection->webview != NULL)
		g_clear_object (&selection->webview);
	selection->webview = g_object_ref (webview);

	editor = webkit_web_view_get_editor_state (webview);
	selection->attrs = webkit_editor_state_get_typing_attributes (editor);
	g_signal_connect (
...
static void xp_editor_selection_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
	...
	switch (property_id) {
		...
		case PROP_UNDERLINE:
			g_value_set_boolean (value, xp_editor_selection_get_underline (selection));
			break;
...
static void xp_editor_selection_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
	...
	switch (property_id) {
		...
		case PROP_UNDERLINE:
			xp_editor_selection_set_underline (selection, g_value_get_boolean (value));
			break;
...
static void xp_editor_selection_finalize (GObject *object)
{
	...
	g_object_unref (selection->webview);
...
static void xp_editor_selection_class_init (XpEditorSelectionClass *klass)
{
	...
	//g_type_add_instance_private (klass, sizeof(XpEditorSelectionPrivate));
}
...
XpEditorSelection * xp_editor_selection_new (WebKitWebView *parent_view)
{
	...
	return g_object_new (XP_TYPE_EDITOR_SELECTION, "webview", parent_view, NULL);
}
...
gboolean xp_editor_selection_get_bold (XpEditorSelection *selection)
{
	...
	return selection->attrs & WEBKIT_EDITOR_TYPING_ATTRIBUTE_BOLD;
}
...
void xp_editor_selection_set_bold (XpEditorSelection *selection, gboolean bold)
{
	...
	webkit_web_view_execute_editing_command (selection->webview, "Bold");
}
...
gboolean xp_editor_selection_get_italic (XpEditorSelection *selection)
{
	...
	return selection->attrs & WEBKIT_EDITOR_TYPING_ATTRIBUTE_ITALIC;
}
...
gboolean xp_editor_selection_get_underline (XpEditorSelection *selection)
{
	...
	return selection->attrs & WEBKIT_EDITOR_TYPING_ATTRIBUTE_UNDERLINE;
}
...
void xp_editor_selection_set_italic (XpEditorSelection *selection, gboolean italic)
{
	...
	webkit_web_view_execute_editing_command (selection->webview, "Italic");
}
...
void xp_editor_selection_set_underline (XpEditorSelection *selection, gboolean underline)
{
	...
	if ((xp_editor_selection_get_underline (selection) ? TRUE : FALSE)
			== (underline ? TRUE : FALSE)) {
		return;
	}

	webkit_web_view_execute_editing_command (selection->webview, "Underline");
}
...
gboolean xp_editor_selection_get_strike_through (XpEditorSelection *selection)
{
	...
	return selection->attrs & WEBKIT_EDITOR_TYPING_ATTRIBUTE_STRIKETHROUGH;
}
...
void xp_editor_selection_set_strike_through (XpEditorSelection *selection, gboolean strike_through)
{
	...
	webkit_web_view_execute_editing_command (selection->webview, "Strikethrough");
}
