...
static void xp_webkit_editor_handle_contents_update (XpWebkitEditor *self, JSGlobalContextRef js_context, JSObjectRef js_object)
{
...
static void xp_webkit_editor_handle_selection_change (XpWebkitEditor *self, JSGlobalContextRef js_context, JSObjectRef js_object)
{
...
static void on_script_message (WebKitUserContentManager *user_content, WebKitJavascriptResult *message, XpWebkitEditor *self)
{
	...
	if (g_strcmp0 (message_name, "ContentsUpdate") == 0)
	{
		if (self->priv->first_load)
			...
			xp_webkit_editor_handle_contents_update (self, js_context, js_object);
	}
	else if (g_strcmp0 (message_name, "SelectionChange") == 0)
		xp_webkit_editor_handle_selection_change (self, js_context, js_object);
}
